# Hi there, I’m Md Tanjil Islam Bappi 👋

I’m an **Infrastructure Engineer** with a deep love for VMware, SAN storage, and disaster‑recovery solutions. I thrive on designing resilient, high‑performance data‑center environments and automating routine operations.

---

## 📚 Professional Summary
Experienced Infrastructure Engineer specialising in VMware virtualization, SAN storage, and disaster recovery across large‑scale, multi‑site enterprise environments. Proven track record managing 220 hosts, ~2,600 VMs, and ~3 PB of storage in regulated financial services. Skilled in PowerShell automation, workload migrations, and cross‑functional delivery. Currently pursuing an MSc in Computer Science (University of Bath) while holding multiple VMware certifications.

---

## 💼 Experience

**Infrastructure Engineer – VMware & Storage**
*OneSavings Bank Plc, Chatham, UK* `May 2023 – Mar 2026`
- Managed 220 VMware hosts & 2,600 VMs across six data‑centers.
- Administered Zerto replication, performed patches & upgrades.
- Built/maintained Dell & HPE storage (≈ 3 PB) and automated 15+ tasks with PowerShell, cutting manual effort by ~35 %.

**Senior System Engineer – Storage & Virtualization**
*Omega Exim Ltd., Dhaka, Bangladesh* `Jun 2015 – Dec 2022`
- Delivered end‑to‑end storage, VMware & backup projects.
- Designed NetApp solutions, managed SAN switches, and led on‑site support for banking clients.

**System Engineer – Network Support Division**
*Flora Ltd., Dhaka, Bangladesh* `Aug 2011 – May 2015`
- Provided enterprise‑wide technical support, deployed HPE & HP hardware, and built campus‑network infrastructure for public universities.

**IS Support Engineer**
*Linde Bangladesh Ltd., Dhaka, Bangladesh* `Mar 2009 – Dec 2009`
- Ran the IT Help Desk for HQ & 19 sales centres; implemented LAN/Server solutions and disaster‑recovery procedures.

---

## 🎓 Education
- **MSc Computer Science** – University of Bath (part‑time), started Sep 2025, UK


---

## 🛠️ Skills
- **Virtualization:** VMware vSphere, vSAN, NSX, SRM, vROPs, Hyper‑V
- **SAN Storage:** NetApp AFF/FAS/E‑Series, HPE MSA/storeonce/3PAR, Dell PowerStore, Unity, Brocade & Cisco MDS
- **Servers & OS:** HPE, Dell, Cisco UCS, IBM, Huawei, Fujitsu; Ubuntu, CentOS, RHEL, Windows Server
- **Automation:** PowerShell, Bash, Ansible (basic)
- **Backup / DR:** Veeam, Commvault, Tape Libraries (HPE/Dell/Oracle)
- **Cloud:** VMware Cloud on AWS, Azure, Oracle Cloud

---

## 📜 Certifications
- VMware Certified Advanced Professional – Network Virtualization (2021)
- VMware Certified Implementation Expert – Data Center Virtualization (2022)
- VMware Certified Professional – Data Center Virtualization (2021)
- VMware Certified Professional – Network Virtualization (2021)
- IELTS Academic – 6.5

---

## 🔗 Connect
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/tanjil360/)

*Feel free to explore my repos for projects on automation, virtualization, and storage engineering.*
